      ...
      <div class="row">
        <div class="col-sm-12 col-md-12 col-lg-10 offset-lg-1">
          <div class="kontakt-footer">
            <div class="wraper">
              <?php if(get_field('nazwa_firmy', 'option')):?>
                <i><?php the_field('tresc', 'option'); ?></i>
              <?php endif; ?>
              <div>
                <?php if(get_field('nazwa_firmy', 'option')):?>
                  <a class="tel" href="tel:<?php the_field('nr_telefonu', 'option'); ?>"><?php the_field('nr_telefonu', 'option'); ?></a>
                <?php endif; ?>
                <?php if(get_field('button_link', 'option') && get_field('button_tresc', 'option')):?>
                  <a href="<?php the_field('button_link', 'option'); ?>" class="btn btn-main"><?php the_field('button_tresc', 'option'); ?></a>
                <?php endif; ?>
              </div>
            </div>
          </div>
        ...
        <div class="col-12 col-sm-12 col-md-12 col-lg-4">
          <address class="info-footer">
            <?php if(get_field('logo-footer', 'option')):
                  $logofooter = get_field('logo-footer', 'option');
            ?>
              <a class="logo-war" href='<?php echo esc_url( home_url( '/' ) ); ?>' title='<?php echo esc_attr( get_bloginfo( 'name', 'display' ) ); ?>' rel='home'>
                <img src="<?php echo esc_url($logofooter['url']); ?>" alt="<?php echo esc_attr($logofooter['alt']); ?>" />
              </a>
            <?php endif; ?>
            <?php if(get_field('nazwa_firmy', 'option')):?>
              <meta itemprop="name" content = "<?php the_field('nazwa_firmy', 'option'); ?>"/>
            <?php endif; ?>
            <?php if(get_field('opis', 'option')):?>
              <span itemprop="description"> <?php the_field('opis', 'option'); ?></span>
            <?php endif; ?>
            <div itemprop="address" itemscope itemtype="http://schema.org/PostalAddress">
              <span><b>Adres firmowy:</b></span>
              <p>
                <?php if(get_field('adres', 'option')):?>
                <span itemprop="streetAddress"><?php the_field('adres', 'option'); ?></span>
                <?php endif; ?>
                <?php if(get_field('adres_miasto', 'option')):?>
                <span itemprop="addressLocality"><?php the_field('adres_miasto', 'option'); ?></span>,
                <?php endif; ?>
                <?php if(get_field('adres_cod_pocztowy', 'option')):?>
                <span itemprop="postalCode"><?php the_field('adres_cod_pocztowy', 'option'); ?></span>
                <?php endif; ?>
              </p>
            </div>
            <?php if(get_field('telefon', 'option')):?>
              <div class="contact">
              <span><b>Tel:</b></span> <a href="tel:<?php the_field('telefon', 'option') ?>"> <?php the_field('telefon', 'option') ?></a>
              </div>
            <?php endif; ?>
            <?php if(get_field('email', 'option')):?>
              <div class="contact">
              <span><b>E-mail:</b></span> <a href="mailto:<?php the_field('email', 'option') ?>"><?php the_field('email', 'option') ?></a>
              </div>
            <?php endif; ?>
            <?php if(get_field('facebook-footer', 'option')):?>
                <div class="contact">
                  <a target="_blank" href="<?php the_field('facebook-footer', 'option') ?>"></a>
                </div>
            <?php endif; ?>
          </address>
        </div>
